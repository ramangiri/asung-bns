...
async function initAddEmployee(){
  ...
  form.addEventListener('submit', async (e)=>{
    ...
    try{
      if(id){
        await api('/api/employees/' + id, { method:'PUT', body: JSON.stringify(employee) });
      }else{
      ...
      msg.textContent = 'Employee saved successfully';
      setTimeout(()=> location.href='employee-list.html', 500);
    }catch(err){
